...
const actions = {
    AUTH_REQUEST: ({commit}, user) => {
        return new Promise((resolve, reject) => {
            ...
            this.default.state.axios({
                url: 'http://' + this.default.state.ip + ':8000/auth',
                data: user,
                method: 'POST'
            })
                .then(resp => {
    ...
    REGISTER_REQUEST: ({commit}, user) => {
        return new Promise((resolve, reject) => {
            ...
            this.default.state.axios({url: 'http://' + this.default.state.ip + ':8000/reg', data: user, method: 'POST'})
                .then(resp => {
                    resolve(resp)
                })
                .catch(err => {
    ...
    SESSION_CHECK: ({commit}) => {
        ...
        this.default.state.axios({url: 'http://' + this.default.state.ip + ':8000/', method: 'GET'})
            .then(resp => {
    ...
    GET_DIALOGS: () => {
        return new Promise((resolve, reject) => {
            ...
            // commit('REGISTER_REQUEST')
            this.default.state.axios({
                url: 'http://' + this.default.state.ip + ':8000/getDialogs', method: 'GET'
            })
                .then(resp => {
                    resolve(resp)
                })
    ...
    SEARCH_USER: ({}, data) =>{
        return new Promise((resolve, reject) => {
            // commit('REGISTER_REQUEST')
            this.default.state.axios({
                url: 'http://' + this.default.state.ip + ':8000/searchUser', data: data, method: 'POST'
            })
                .then(resp => {
                    resolve(resp)
                })
...
const getters = {
    ...
    getUsername2: state => state.username2,
